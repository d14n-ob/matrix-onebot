# matrix-onebot

![OneBot 12] ![build]

Matrix 平台的 OneBot 实现，使 OneBot 应用可以对接到 Matrix。

本项目使用 [matrix-rust-sdk](https://github.com/matrix-org/matrix-rust-sdk) 与 [walle-core](https://github.com/onebot-walle/walle-core) 构建。

## 社区

Matrix: [#onebot:matrix.meowl.cc](https://matrix.to/#/#onebot:matrix.meowl.cc)

## 许可证

[Mozilla Public License, Version 2.0](LICENSE)

[OneBot 12]:https://img.shields.io/badge/OneBot-12-black?logo=data:image/png;base64,
[build]:https://img.shields.io/github/actions/workflow/status/d14n-ob/matrix-onebot/compile.yml